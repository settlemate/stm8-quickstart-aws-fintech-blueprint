...
TIP: If you get an error when trying to connect about resolving Client VPN endpoint DNS or hostname, you may need to change one more line in the `.ovpn` file. The quick fix and the reason for the extra step are https://docs.aws.amazon.com/vpn/latest/clientvpn-admin/troubleshooting.html#resolve-host-name[detailed in the Client VPN Troubleshooting section].
