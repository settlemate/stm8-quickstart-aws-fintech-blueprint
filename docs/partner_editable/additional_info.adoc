...
For example, the Operational Best Practices for the National Institute of Standards and Technology (NIST) Cybersecurity Framework (CSF) conformance pack comes with 93 rules. One of those rules—*encrypted-volumes-conformance-pack*, highlighted in <<conformance_pack0>>—checks whether attached Amazon Elastic Block Store (Amazon EBS) volumes are encrypted. 
